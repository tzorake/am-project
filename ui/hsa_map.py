...
class HSAMap(QtWidgets.QWidget):
    ...
    def on_calculateButton_clicked(self):
        ...
        with open(color_theme_path) as file:
            color_theme_config = json.load(file)

        active_theme = color_theme_config["active_theme"]
        color_themes = color_theme_config["color_themes"]
        color_theme = color_themes[active_theme]

        def color(c):
            return cl.to_rgb(c)

        colors = [
            color(color_theme["CHAOS"]), 
            color(color_theme["FAKE"]),
            color(color_theme["UNDEF"]),
            color(color_theme["SP_IND_FREQ"]),
            color(color_theme["IND_FREQ"]),
            color(color_theme["HARMONIC"])
        ]
        ...
        map_ax.imshow(values, cmap = cmap)
        self.map.draw()